...
function Instagram({ p }: { p: PostDraft }) {
  ...
  return (
    <div className="w-[360px] bg-white rounded-lg ring-1 ring-slate-200 overflow-hidden text-[13px]">
      <div className="flex items-center gap-2.5 px-3 py-2.5">
        <div className={`w-8 h-8 rounded-full ${AVATAR} text-white text-[10px] font-bold flex items-center justify-center`}>{initials(h)}</div>
      ...
      {p.image_url
        ...
        : <div className="w-full aspect-square bg-slate-100 flex items-center justify-center text-slate-300 text-[12px]">No image yet</div>}
...
function XCard({ p }: { p: PostDraft }) {
  const h = p.handle || '@yourbrand';
  return (
    <div className="w-[420px] bg-white rounded-xl ring-1 ring-slate-200 p-3.5 text-[13.5px]">
      <div className="flex gap-2.5">
        <div className={`w-10 h-10 rounded-full ${AVATAR} text-white text-[11px] font-bold flex items-center justify-center shrink-0`}>{initials(h)}</div>
...
function Facebook({ p }: { p: PostDraft }) {
  ...
  return (
    <div className="w-[420px] bg-white rounded-lg ring-1 ring-slate-200 text-[13.5px]">
      <div className="flex items-center gap-2.5 px-3.5 py-3">
        <div className={`w-10 h-10 rounded-full ${AVATAR} text-white text-[11px] font-bold flex items-center justify-center`}>{initials(h)}</div>
        <div>
          <div className="font-semibold text-slate-900">{h.replace(/^@/, '')}</div>
          <div className="flex items-center gap-1 text-[11px] text-slate-400">Just now · <Globe className="w-3 h-3" /></div>
        </div>
        <MoreHorizontal className="w-4 h-4 text-slate-500 ml-auto" />
      </div>
      ...
      <div className="flex items-center justify-around px-3.5 py-2 border-t border-slate-100 text-slate-500 text-[12.5px] font-medium">
  ...
  return (
    <div className="w-[420px] bg-white rounded-lg ring-1 ring-slate-200 text-[13.5px]">
      <div className="flex items-start gap-2.5 px-3.5 py-3">
        <div className={`w-11 h-11 rounded-full ${AVATAR} text-white text-[11px] font-bold flex items-center justify-center`}>{initials(h)}</div>
        <div>
          <div className="font-semibold text-slate-900">{h.replace(/^@/, '')}</div>
          <div className="text-[11px] text-slate-400">1,234 followers · Just now</div>
        </div>
        <MoreHorizontal className="w-4 h-4 text-slate-500 ml-auto" />
      </div>
      <p className="px-3.5 pb-3 text-slate-900 whitespace-pre-wrap">{p.content || 'Your post…'}</p>
