...
import {
  ...
  FileStack, Globe2, FolderOpen, Waypoints,
} from 'lucide-react';
...
export default function NavRail({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return (
    <aside className="w-[17rem] h-full shrink-0 bg-canvas flex flex-col">
      <DropdownMenu open={wsOpen} onOpenChange={onSwitcherOpenChange}>
        <DropdownMenuTrigger asChild>
          <button
            className="w-full h-16 flex items-center gap-2 px-4 outline-none hover:bg-surface-hover data-[state=open]:bg-surface-hover transition-colors">
            {logo ? (
        ...
        <DropdownMenuContent align="start" sideOffset={4} className="w-[16.5rem]">
