    ...
    <div className="space-y-2.5">
      {stages.map((s) => (
        <div key={s.key} className="flex items-center gap-3">
          <span className="w-20 shrink-0 text-[12px] font-medium text-slate-500">{s.label}</span>
          <div className="flex-1 h-6 rounded-md bg-slate-50 ring-1 ring-slate-100 overflow-hidden">
            <div className="h-full rounded-md transition-all duration-500" style={{ width: `${Math.max(3, (s.count / max) * 100)}%`, background: s.tone }} />
          </div>
          <span className="w-9 shrink-0 text-right text-[13px] font-bold tabular-nums text-slate-700">{s.count}</span>
        </div>
      ))}
    </div>
  );
