    ...
    <div className="mt-3 rounded-xl ring-1 ring-subtle bg-surface-sunken/80 [background-image:radial-gradient(#d3dce6_1px,transparent_1px)] [background-size:14px_14px] p-3.5 overflow-x-auto">
      <div className="flex items-center min-w-max">
        ...
        {a.conditions.length > 0 && (
          <>
            ...
            <Node icon={Filter} title="Filter" sub={`${a.conditions.length} condition${a.conditions.length > 1 ? 's' : ''}`}
              tone="bg-surface-hover text-secondary" ring="ring-subtle" onClick={onEdit} />
          </>
        )}
        {(a.actions || []).map((ac, i) => {
        ...
        {(a.actions || []).length === 0 && (
          <>
            <Edge />
            <Node icon={Zap} title="No action yet" sub="Click to add one" tone="bg-surface-hover text-tertiary" ring="ring-subtle" onClick={onEdit} />
          </>
        )}
      </div>
    </div>
