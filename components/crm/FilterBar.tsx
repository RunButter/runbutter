    ...
    <div className="shrink-0 flex flex-wrap items-center gap-1.5 px-4 py-2 border-b border-subtle bg-surface-sunken/40">
      {usableFacets.map((f) => (
        <select key={f.key} value={value.facets[f.key] || ''} onChange={(e) => setFacet(f.key, e.target.value)}
          className={`h-7 px-2 text-[12px] rounded-md bg-surface ring-1 outline-none focus:ring-2 focus:ring-accent/30 capitalize ${value.facets[f.key] ? 'ring-accent/30 text-accent font-semibold' : 'ring-subtle text-secondary'}`}>
      ...
      {dateField && (
        <div className="flex items-center gap-1 text-[12px] text-secondary ml-0.5">
          ...
          <input type="date" value={value.from} onChange={(e) => onChange({ ...value, from: e.target.value })}
            className="h-7 px-1.5 text-[12px] rounded-md bg-surface ring-1 ring-subtle outline-none focus:ring-2 focus:ring-accent/30" />
      ...
      {hasActiveFilters(value) && (
        <button onClick={() => onChange(EMPTY_FILTERS)}
          className="h-7 px-2 inline-flex items-center gap-1 text-[12px] font-medium text-secondary hover:text-danger">
