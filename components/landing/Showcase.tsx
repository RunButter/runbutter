  ...
  return (
    <div className="bg-surface rounded-lg border border-subtle p-2 mb-2">
      <div className="flex items-center gap-1.5">
        <span className="w-3.5 h-3.5 rounded-full bg-accent/30 shrink-0" />
      ...
      {amount && <div className="text-[10px] font-medium text-success mt-1 pl-5">{amount}</div>}
    </div>
  );
}
...
function FinanceMock() {
  const kpis = [
    { label: 'Revenue', value: '$38,000', tone: 'text-success' },
    { label: 'Costs', value: '$19,000', tone: 'text-secondary' },
    { label: 'Net', value: '$19,000', tone: 'text-success' },
  ];
  return (
    <MockWindow>
      <div className="grid grid-cols-3 gap-2 mb-3">
        {kpis.map((k) => (
          <div key={k.label} className="rounded-lg border border-subtle p-2">
            <div className="text-[9px] font-medium uppercase tracking-wide text-tertiary">{k.label}</div>
            <div className={`text-base font-medium tabular-nums ${k.tone}`}>{k.value}</div>
          </div>
        ))}
      </div>
      <div className="flex items-center gap-3 mb-1 text-[10px] font-medium">
        <span className="inline-flex items-center gap-1 text-secondary"><span className="w-2 h-2 rounded-sm bg-success" /> Revenue</span>
...
function RecruitingMock() {
  const rows: [string, string, string, string, string][] = [
    ...
    ['Sara Lindqvist', 'Product Designer', '74', 'Screening', 'bg-warning/10 text-warning'],
  ];
  return (
    <MockWindow>
      <div className="rounded-lg border border-subtle overflow-hidden">
        ...
        {rows.map((r) => (
          <div key={r[0]} className="grid grid-cols-[1fr_auto_auto] gap-3 px-3 h-[52px] items-center border-t border-subtle">
            ...
            <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium ${r[4]}`}>{r[3]}</span>
          </div>
        ))}
      </div>
    </MockWindow>
      ...
      <div className="grid md:grid-cols-[1.1fr_1.4fr] gap-4">
        <div className="space-y-3">
          <div className="grid grid-cols-3 gap-2">
            {[['Visitors', '2,480', 'text-primary'], ['Views', '6,120', 'text-primary'], ['Live', '7', 'text-success']].map((k) => (
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-10 lg:gap-14 items-center">
      <div className={reverse ? 'lg:order-2' : ''}>
        <div className={`inline-block text-2xs font-medium uppercase tracking-wider mb-3 text-accent`}>{eyebrow}</div>
        ...
        <ul className="mt-5 space-y-2.5">
          {bullets.map((b) => (
            <li key={b} className="flex items-start gap-2 text-[14px] text-secondary"><Check className="w-4 h-4 text-success shrink-0 mt-0.5" />{b}</li>
          ))}
        </ul>
      </div>
...
export default function Showcase() {
  return (
    <section className="max-w-6xl mx-auto px-6 py-20 space-y-20">
      <Row
        eyebrow="Sales CRM" tone="text-accent bg-accent/10"
      ...
      <Row
        reverse
        eyebrow="Finance" tone="text-accent bg-accent/10"
      ...
      <Row
        eyebrow="Recruiting & HR" tone="text-accent bg-accent/10"
