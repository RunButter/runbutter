...
export async function POST(req: Request) {
  try {
    ...
    const total = items.length ? items.reduce((s, it) => s + (+it.line_total || 0), 0) : (+d.amount || 0);
    ...
    const rows = items.map((it) => `
      <tr>
        <td style="padding:8px 0;border-bottom:1px solid #EEE;color:#333;">${esc(it.description || it.product || 'Item')}</td>
