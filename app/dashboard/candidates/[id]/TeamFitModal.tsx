    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 backdrop-blur-sm p-4" onClick={onClose}>
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center justify-between p-6 border-b border-gray-100">
                    <div>
                        <h3 className="text-xl font-black text-gray-900 flex items-center gap-2">
                            <Sparkles className="w-5 h-5 text-primary-600" /> Team Fit Simulator
                        </h3>
                        <p className="text-sm text-gray-500">See how this candidate&apos;s personality meshes with an existing team.</p>
                    </div>
                ...
                <div className="p-6">
                    {loading ? (
                        <div className="py-20 flex items-center justify-center"><Loader2 className="w-8 h-8 text-primary-600 animate-spin" /></div>
                    ) : !results?.personality_data ? (
                        <p className="py-12 text-center text-gray-400">This candidate has no personality data to simulate.</p>
                    ) : teams.length === 0 ? (
                        <p className="py-12 text-center text-gray-400">No other assessed candidates yet to build a team profile from.</p>
                    ) : (
                        <>
                            <div className="mb-5">
                                <label className="block text-xs font-bold text-gray-600 mb-1">Mock-insert into team</label>
                                <select value={teamKey} onChange={(e) => setTeamKey(e.target.value)}
                                    className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg bg-white focus:ring-2 focus:ring-primary-500 outline-none">
                                ...
                                <div>
                                    <div className="flex items-center gap-4 mb-4">
                                        <div className="text-center">
                                            <div className={`text-5xl font-black ${synergyColor}`}>
                                                {analysis ? analysis.synergy : 0}<span className="text-2xl">%</span>
                                            </div>
                                            <div className="text-[10px] font-black uppercase tracking-widest text-gray-400">Synergy</div>
                                        </div>
                                        {analysis && (
                                        ...
                                        {analysis && analysis.deltas.map((d) => (
                                            <div key={d.key} className="flex items-center justify-between text-xs">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <span className="font-mono text-gray-400">{d.cand} vs {d.team}</span>
                                                    <span className={`px-2 py-0.5 rounded-full font-bold ${tagColor(d.tag)}`}>{d.tag}</span>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
