  ...
  return (
    <>
      <header className="h-12 shrink-0 flex items-center gap-3 px-4 border-b border-subtle">
        ...
        <span className="text-[11px] font-semibold text-tertiary bg-surface-hover rounded-md px-1.5 py-0.5 tabular-nums">
          {publicCount} public
        </span>
        <Link href="/settings/branding"
          className="ml-auto h-7 px-2.5 inline-flex items-center gap-1.5 rounded-md text-[12px] font-medium text-secondary ring-1 ring-subtle hover:bg-surface-sunken">
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="rounded-xl bg-surface ring-1 ring-subtle shadow-card p-5">
                ...
                <p className="text-[12px] text-tertiary mb-3.5">
                ...
                {activeRoles.length === 0 ? (
                  <EmptyState icon={Briefcase} title="No open positions"
                    ...
                    action={<Link href="/dashboard/positions/new"
                      className="h-8 px-3 inline-flex items-center rounded-lg text-[13px] font-semibold text-inverse-fg bg-inverse hover:bg-inverse/90">
                      New position
                    </Link>} />
                ) : (
                    ...
                    {activeRoles.map((r) => (
                      <li key={r.id} className="flex items-center gap-3 py-2.5">
                        <div className="min-w-0 flex-1">
                          <p className="text-[13px] font-medium text-primary truncate">{r.title}</p>
                          {r.department && <p className="text-[11px] text-tertiary truncate">{r.department}</p>}
                        </div>
                        ...
                        <button onClick={() => togglePublished(r)} disabled={busyId === r.id}
                          className={`h-7 px-2.5 inline-flex items-center gap-1.5 rounded-md text-[12px] font-semibold ring-1 disabled:opacity-50 ${
                            r.is_published
