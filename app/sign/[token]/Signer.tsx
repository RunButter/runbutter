    ...
    return (
      <div className="flex-1 flex items-center justify-center px-6">
        <div className="w-full max-w-md rounded-xl bg-surface border border-subtle p-8 text-center">
          ...
          <p className="mt-2 text-sm text-secondary leading-relaxed">Your signature on <b>{title}</b> is recorded. Once everyone has signed, the completed PDF is emailed to you.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="flex-1 grid md:grid-cols-[1fr_380px] min-h-0">
      ...
      <div className="bg-surface-sunken min-h-[40vh] md:min-h-0">
        {docUrl ? (
          ...
          <div className="p-8 text-center text-tertiary text-sm">Document preview unavailable.</div>
        )}
      </div>
      ...
      <div className="border-l border-subtle p-6 flex flex-col gap-4 overflow-auto">
        ...
        <div className="flex gap-1 p-1 rounded-lg bg-surface-sunken">
          <button onClick={() => setMode('draw')} className={`flex-1 h-8 rounded-md text-xs font-semibold inline-flex items-center justify-center gap-1.5 ${mode === 'draw' ? 'bg-surface text-primary shadow-sm' : 'text-tertiary'}`}><PenLine className="w-3.5 h-3.5" /> Draw</button>
          <button onClick={() => setMode('type')} className={`flex-1 h-8 rounded-md text-xs font-semibold inline-flex items-center justify-center gap-1.5 ${mode === 'type' ? 'bg-surface text-primary shadow-sm' : 'text-tertiary'}`}><Type className="w-3.5 h-3.5" /> Type</button>
        </div>
        ...
        <p className="text-2xs text-tertiary text-center">Signed with RunButter · your IP and time are recorded for the audit trail.</p>
      </div>
    </div>
  );
}
