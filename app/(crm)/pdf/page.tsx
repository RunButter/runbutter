    ...
    <>
      <header className="h-12 shrink-0 flex items-center gap-3 px-4 border-b border-subtle">
        <h1 className="text-sm font-semibold text-primary flex items-center gap-2">
          <FileText className="w-4 h-4 text-accent" /> PDF editor
        </h1>
        ...
        {pages.length > 0 && (
          <span className="text-[11px] text-tertiary tabular-nums">{pages.length} pages · {files.length} file{files.length === 1 ? '' : 's'}</span>
        )}
        <div className="ml-auto flex items-center gap-2">
          <button onClick={open} className="h-8 px-3 inline-flex items-center gap-1.5 rounded-lg text-[13px] font-medium text-secondary ring-1 ring-subtle hover:bg-surface-sunken">
            Add PDFs
          </button>
          {pages.length > 0 && (
      ...
      <div {...getRootProps()} className="flex-1 overflow-y-auto p-4 outline-none">
        <input {...getInputProps()} />
        ...
        {pages.length === 0 ? (
          <div className={`max-w-xl mx-auto mt-10 rounded-xl border border-dashed p-10 text-center transition-colors ${
            ...
            <EmptyState icon={FileText}
              ...
              action={<button onClick={open} className="h-8 px-3 rounded-lg text-[13px] font-semibold text-inverse-fg bg-inverse hover:bg-inverse/90">Choose files</button>} />
          </div>
        ) : (
          <div className="max-w-6xl mx-auto space-y-3">
            ...
            <div className="sticky top-0 z-10 -mx-1 px-1 py-2 bg-canvas/90 backdrop-blur-sm flex flex-wrap items-center gap-1">
              <button onClick={() => setSelected(allSelected ? new Set() : new Set(pages.map((p) => p.key)))}
                className="h-7 px-2 inline-flex items-center gap-1.5 rounded-md text-[12px] font-medium text-secondary hover:bg-surface-hover">
              ...
              <button onClick={() => remove()} title={`Delete ${scope}`}
                className="h-7 px-2 inline-flex items-center gap-1.5 rounded-md text-[12px] font-medium text-secondary hover:text-danger hover:bg-danger/10">
                <Trash2 className="w-3.5 h-3.5" /> Delete
              </button>
              <button onClick={splitEach} disabled={!!busy} title={`One file per page (${scope})`}
                className="h-7 px-2 inline-flex items-center gap-1.5 rounded-md text-[12px] font-medium text-secondary hover:bg-surface-hover disabled:opacity-40">
              ...
              <input value={wmText} onChange={(e) => setWmText(e.target.value)} placeholder="Watermark…"
                ...
                className="h-7 w-32 px-2 text-[12px] rounded-md bg-surface-hover text-primary placeholder:text-tertiary outline-none focus:ring-2 focus:ring-accent/30" />
              ...
              <span className="ml-auto flex items-center gap-1">
                <span className="text-[11px] text-tertiary tabular-nums px-1">
                  {selected.size ? `${selected.size} selected` : `${pages.length} pages`}
                </span>
                <button onClick={reset} className="h-7 px-2 inline-flex items-center gap-1.5 rounded-md text-[12px] font-medium text-tertiary hover:bg-surface-hover">
            ...
            <div className="pt-3 border-t border-subtle">
              <label className="inline-flex items-center gap-1.5 text-[12px] text-tertiary hover:text-secondary cursor-pointer">
