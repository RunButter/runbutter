      ...
      <header className="h-12 shrink-0 flex items-center gap-3 px-4 border-b border-subtle">
        ...
        <div className="ml-auto flex items-center gap-1.5">
          ...
          <button onClick={() => setForm({ initial: { txn_date: new Date().toISOString().slice(0, 10), status: 'posted', method: 'transfer' } })} disabled={!canEdit}
            className="h-7 px-2.5 inline-flex items-center gap-1.5 rounded-md text-[12px] font-semibold text-white bg-accent hover:bg-accent/90 shadow-sm disabled:opacity-40" title={!canEdit ? 'Sign in to add' : ''}><Plus className="w-3.5 h-3.5" /> New</button>
        </div>
      </header>
          ...
          {showCat && (
            <div className="absolute bottom-9 left-0 w-56 bg-surface text-primary rounded-lg border border-subtle shadow-popover p-2" onClick={(e) => e.stopPropagation()}>
              ...
              <button onClick={() => { if (cat.trim()) { onCategorize(cat.trim()); setShowCat(false); setCat(''); } }}
                className="mt-1.5 w-full h-7 rounded-md text-[12px] font-semibold text-white bg-accent hover:bg-accent/90">Apply</button>
            </div>
          )}
        </div>
      )}
                  ...
                  <div key={`${s.kind}-${s.id}`} className="flex items-center gap-2 rounded-lg ring-1 ring-subtle p-2.5 hover:ring-strong">
                    ...
                    {canEdit && (
                      <button onClick={() => doMatch(s.kind, s.id)} disabled={busy === s.id}
                        className="h-7 px-2.5 rounded-md text-[12px] font-semibold text-white bg-accent hover:bg-accent/90 inline-flex items-center gap-1.5 disabled:opacity-50">
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-[2px] p-4" onClick={onClose}>
      <div className="w-full max-w-sm bg-surface rounded-xl ring-1 ring-subtle shadow-2xl animate-in fade-in zoom-in-95 duration-150" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-end gap-2 p-3 border-t border-subtle">
          ...
          <button onClick={save} disabled={busy} className="h-8 px-3 rounded-md text-[13px] font-semibold text-white bg-accent hover:bg-accent/90 inline-flex items-center gap-1.5 disabled:opacity-50">{busy && <Loader2 className="w-3.5 h-3.5 animate-spin" />} Create</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-[2px] p-4" onClick={onClose}>
      <div className="w-full max-w-lg max-h-[85vh] flex flex-col bg-surface rounded-xl ring-1 ring-subtle shadow-2xl animate-in fade-in zoom-in-95 duration-150" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="shrink-0 flex items-center justify-end gap-2 p-3 border-t border-subtle">
          ...
          {step === 'map' && (
            <button onClick={run} disabled={busy} className="h-8 px-3 rounded-md text-[13px] font-semibold text-white bg-accent hover:bg-accent/90 inline-flex items-center gap-1.5 disabled:opacity-50">{busy && <Loader2 className="w-3.5 h-3.5 animate-spin" />} Import {rows.length} rows</button>
          )}
          {step === 'done' && <button onClick={onDone} className="h-8 px-3 rounded-md text-[13px] font-semibold text-white bg-accent hover:bg-accent/90">Done</button>}
        </div>
      </div>
    </div>
  );
