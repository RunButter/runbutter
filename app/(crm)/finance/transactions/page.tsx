...
const STATUS_PILL: Record<string, string> = {
  ...
  excluded: 'bg-slate-100 text-slate-400 ring-slate-200/60',
};
  ...
  return (
    <>
      <header className="h-12 shrink-0 flex items-center gap-3 px-4 border-b border-slate-200/70">
        <h1 className="text-sm font-bold text-slate-800">Transactions</h1>
        ...
        <div className="ml-auto flex items-center gap-1.5">
          <div className="relative">
            ...
            <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search…"
              className="h-7 w-40 pl-7 pr-2 text-[12px] rounded-md bg-white ring-1 ring-slate-200 focus:ring-2 focus:ring-primary-500 outline-none" />
          </div>
          <div className="flex items-center gap-0.5 p-0.5 rounded-lg bg-slate-100 ring-1 ring-slate-200/60">
            {PERIODS.map((p) => (
              <button key={p.label} onClick={() => setMonths(p.months)}
                className={`h-6 px-2 rounded-md text-[11px] font-bold transition-colors ${months === p.months ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>{p.label}</button>
            ))}
          </div>
          <button onClick={() => exportCsv(filtered)} disabled={filtered.length === 0}
            className="h-7 px-2 inline-flex items-center gap-1.5 rounded-md text-[12px] font-medium text-slate-600 ring-1 ring-slate-200 hover:bg-slate-50 disabled:opacity-40"><Download className="w-3.5 h-3.5" /> Export</button>
          <button onClick={() => setImporting(true)} disabled={!canEdit}
            className="h-7 px-2 inline-flex items-center gap-1.5 rounded-md text-[12px] font-medium text-slate-600 ring-1 ring-slate-200 hover:bg-slate-50 disabled:opacity-40" title={!canEdit ? 'Sign in to import' : ''}><Upload className="w-3.5 h-3.5" /> Import</button>
          <button onClick={() => setForm({ initial: { txn_date: new Date().toISOString().slice(0, 10), status: 'posted', method: 'transfer' } })} disabled={!canEdit}
            className="h-7 px-2.5 inline-flex items-center gap-1.5 rounded-md text-[12px] font-bold text-white bg-primary-600 hover:bg-primary-700 shadow-sm disabled:opacity-40" title={!canEdit ? 'Sign in to add' : ''}><Plus className="w-3.5 h-3.5" /> New</button>
        </div>
      </header>

      {/* Account tabs */}
      <div className="shrink-0 flex items-center gap-2 px-4 py-2 border-b border-slate-200/70 overflow-x-auto">
        <button onClick={() => setAccount(null)}
          className={`h-9 shrink-0 px-3 rounded-lg text-left ring-1 transition-colors ${account === null ? 'bg-white ring-slate-300 shadow-sm' : 'ring-slate-200/60 hover:bg-slate-50'}`}>
          <div className="text-[12px] font-bold text-slate-800 leading-tight">All accounts</div>
          <div className="text-[11px] tabular-nums text-slate-500 leading-tight">{money(totalCash)}</div>
        </button>
        {accounts.map((a) => (
          <button key={a.id} onClick={() => setAccount(a.id)}
            className={`h-9 shrink-0 px-3 rounded-lg text-left ring-1 transition-colors ${account === a.id ? 'bg-white ring-slate-300 shadow-sm' : 'ring-slate-200/60 hover:bg-slate-50'}`}>
            <div className="text-[12px] font-bold text-slate-800 leading-tight flex items-center gap-1.5"><Landmark className="w-3 h-3 text-slate-400" /> {a.name}</div>
            <div className={`text-[11px] tabular-nums leading-tight ${a.balance < 0 ? 'text-rose-600' : 'text-slate-500'}`}>{money(a.balance)}</div>
          </button>
        ))}
        <button onClick={() => setAddingAccount(true)} disabled={!canEdit}
          className="h-9 shrink-0 px-2.5 rounded-lg text-[12px] font-semibold text-slate-500 ring-1 ring-dashed ring-slate-300 hover:bg-slate-50 disabled:opacity-40 inline-flex items-center gap-1.5" title={!canEdit ? 'Sign in to add' : ''}><Plus className="w-3.5 h-3.5" /> Account</button>
      </div>
      ...
      <div className="shrink-0 grid grid-cols-2 lg:grid-cols-4 gap-3 px-4 py-3">
        {[
          ...
          { label: 'To reconcile', value: String(summary.unreconciled), tone: summary.unreconciled > 0 ? 'text-amber-600' : 'text-slate-400', icon: Link2 },
        ].map((c) => (
          <div key={c.label} className="rounded-xl bg-white ring-1 ring-slate-200/60 p-3.5">
            ...
            <div className={`mt-1 text-xl font-black tabular-nums ${c.tone}`}>{c.value}</div>
          </div>
        ))}
      </div>
        ...
        {loading ? (
          ...
          <div className="rounded-xl bg-white ring-1 ring-slate-200/60 overflow-hidden">
            <table className="w-full text-[13px] border-separate border-spacing-0">
              <thead>
                <tr>
                  <th className="w-9 px-3 h-9 bg-slate-50/60 border-b border-slate-200/70">
                    <input type="checkbox" checked={allShownSelected} onChange={toggleAll} className="rounded border-slate-300 accent-primary-600 cursor-pointer" />
                  </th>
                  {['Date', 'Description', 'Category', 'Account', 'Status', 'Reconciled', 'Amount'].map((h, i) => (
                    <th key={h} className={`px-3 h-9 bg-slate-50/60 text-[11px] font-semibold uppercase tracking-wider text-slate-400 border-b border-slate-200/70 ${i === 6 ? 'text-right' : 'text-left'}`}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtered.map((r) => {
                  ...
                  return (
                    <tr key={r.id} onClick={() => setDrawer(r)}
                      className={`group cursor-pointer transition-colors ${on ? 'bg-primary-50/40' : 'hover:bg-slate-50/70'} ${excluded ? 'opacity-50' : ''}`}>
                      <td className="px-3 h-[44px] border-b border-slate-100" onClick={(e) => e.stopPropagation()}>
                      ...
                      <td className="px-3 h-[44px] border-b border-slate-100 text-slate-500 tabular-nums whitespace-nowrap">{fmtDate(r.txn_date)}</td>
                      <td className="px-3 h-[44px] border-b border-slate-100 font-semibold text-slate-800 max-w-[280px] truncate">{r.description || '—'}</td>
                      <td className="px-3 h-[44px] border-b border-slate-100">
                        {r.category
                          ? <span className="inline-flex items-center px-1.5 py-0.5 rounded-md text-[11px] font-semibold ring-1 bg-slate-50 text-slate-600 ring-slate-200/60 capitalize">{r.category}</span>
                          : <span className="text-[12px] text-slate-300">Uncategorized</span>}
                      </td>
                      <td className="px-3 h-[44px] border-b border-slate-100 text-slate-500 truncate max-w-[140px]">{r.account || '—'}</td>
                      <td className="px-3 h-[44px] border-b border-slate-100">
                      ...
                      <td className="px-3 h-[44px] border-b border-slate-100">
                        {r.match
                          ? <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-primary-700"><Link2 className="w-3 h-3" /> {r.match}</span>
                          : <span className="text-[11px] text-slate-300">—</span>}
                      </td>
                      <td className={`px-3 h-[44px] border-b border-slate-100 text-right tabular-nums font-bold ${r.amount < 0 ? 'text-rose-600' : 'text-emerald-600'}`}>{money(r.amount)}</td>
                    </tr>
                  );
                })}
                {filtered.length === 0 && (
                  <tr><td colSpan={8} className="px-3 py-14 text-center text-slate-400">No transactions in this period. Import a bank statement or add one.</td></tr>
                )}
              </tbody>
            </table>
          </div>
...
function BulkBar({ count, canEdit, categorySuggestions, onClear, onCategorize, onStatus, onExport, onDelete }: {
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 flex items-center gap-1 bg-slate-900 text-white rounded-xl shadow-2xl shadow-slate-900/30 pl-3 pr-1.5 py-1.5 text-[13px] animate-in fade-in slide-in-from-bottom-2 duration-150">
      ...
      <button onClick={onClear} className="text-white/50 hover:text-white text-[12px] font-medium ml-1 mr-1">clear</button>
      <span className="w-px h-5 bg-white/15" />
      {canEdit && (
        <div className="relative">
          <button onClick={() => setShowCat((s) => !s)} className="h-7 px-2.5 inline-flex items-center gap-1.5 rounded-md font-semibold hover:bg-white/10"><Tag className="w-3.5 h-3.5" /> Categorize</button>
          {showCat && (
            <div className="absolute bottom-9 left-0 w-56 bg-white text-slate-800 rounded-lg ring-1 ring-slate-200 shadow-xl p-2" onClick={(e) => e.stopPropagation()}>
              <input autoFocus list="bulk-cats" value={cat} onChange={(e) => setCat(e.target.value)} placeholder="Category…"
                ...
                className="w-full h-8 px-2 text-[13px] rounded-md ring-1 ring-slate-200 focus:ring-2 focus:ring-primary-500 outline-none" />
              ...
              <button onClick={() => { if (cat.trim()) { onCategorize(cat.trim()); setShowCat(false); setCat(''); } }}
                className="mt-1.5 w-full h-7 rounded-md text-[12px] font-bold text-white bg-primary-600 hover:bg-primary-700">Apply</button>
            </div>
          )}
        </div>
      )}
      {canEdit && <button onClick={() => onStatus('posted')} className="h-7 px-2.5 inline-flex items-center gap-1.5 rounded-md font-semibold hover:bg-white/10"><Check className="w-3.5 h-3.5" /> Posted</button>}
      {canEdit && <button onClick={() => onStatus('excluded')} className="h-7 px-2.5 inline-flex items-center gap-1.5 rounded-md font-semibold hover:bg-white/10"><Ban className="w-3.5 h-3.5" /> Exclude</button>}
      <button onClick={onExport} className="h-7 px-2.5 inline-flex items-center gap-1.5 rounded-md font-semibold hover:bg-white/10"><Download className="w-3.5 h-3.5" /> Export</button>
      {canEdit && <button onClick={onDelete} className="h-7 px-2.5 inline-flex items-center gap-1.5 rounded-md font-semibold text-rose-300 hover:bg-rose-500/20"><Trash2 className="w-3.5 h-3.5" /> Delete</button>}
      <button onClick={onClear} aria-label="Close" className="h-7 w-7 inline-flex items-center justify-center rounded-md hover:bg-white/10"><X className="w-4 h-4" /></button>
    </div>
  );
}
...
function ReconcileDrawer({ txn, privy, canEdit, categorySuggestions, onClose, onChanged }: {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex justify-end bg-slate-900/30 backdrop-blur-[1px]" onClick={onClose}>
      <div className="w-full max-w-sm h-full bg-white shadow-2xl ring-1 ring-slate-200/70 flex flex-col animate-in slide-in-from-right duration-150" onClick={(e) => e.stopPropagation()}>
        <div className="h-12 shrink-0 flex items-center justify-between px-4 border-b border-slate-200/70">
          <h2 className="text-sm font-bold text-slate-800">Transaction</h2>
          <button onClick={onClose} className="p-1.5 rounded-md text-slate-400 hover:bg-slate-100"><X className="w-4 h-4" /></button>
        </div>

        <div className="flex-1 overflow-y-auto p-4 space-y-5">
          <div>
            <div className="text-[15px] font-bold text-slate-800">{txn.description || '—'}</div>
            <div className={`text-2xl font-black tabular-nums ${out ? 'text-rose-600' : 'text-emerald-600'}`}>{money(txn.amount)}</div>
            <div className="text-[12px] text-slate-400 mt-0.5">{fmtDate(txn.txn_date)} · {txn.account || 'No account'} · {txn.method.replace(/_/g, ' ')}</div>
          </div>
          ...
          {canEdit ? (
            <div className="grid grid-cols-2 gap-3">
              ...
              <label className="block">
                <span className="block text-[12px] font-semibold text-slate-600 mb-1">Status</span>
                <select value={status} onChange={(e) => setStatus(e.target.value)}
                  className="w-full h-9 px-2 text-[13px] rounded-md bg-white ring-1 ring-slate-200 focus:ring-2 focus:ring-primary-500 outline-none capitalize">
                  {['posted', 'pending', 'excluded'].map((s) => <option key={s} value={s}>{s}</option>)}
                </select>
              </label>
              <button onClick={saveEdits} disabled={busy === 'save'} className="col-span-2 h-8 rounded-md text-[13px] font-semibold text-slate-700 ring-1 ring-slate-200 hover:bg-slate-50 inline-flex items-center justify-center gap-1.5 disabled:opacity-50">
          ...
          <div>
            ...
            {txn.match ? (
              <div className="rounded-lg ring-1 ring-primary-200/60 bg-primary-50/60 p-3">
                <div className="flex items-center gap-1.5 text-[13px] font-semibold text-primary-700"><Link2 className="w-3.5 h-3.5" /> {txn.match}</div>
                <p className="text-[12px] text-slate-500 mt-1">Linked and marked paid.</p>
                {canEdit && <button onClick={unmatch} disabled={busy === 'unmatch'} className="mt-2 h-7 px-2.5 rounded-md text-[12px] font-semibold text-slate-600 ring-1 ring-slate-200 hover:bg-white inline-flex items-center gap-1.5 disabled:opacity-50">{busy === 'unmatch' && <Loader2 className="w-3 h-3 animate-spin" />} Unmatch</button>}
              </div>
            ) : suggestions === null ? (
              <div className="h-16 flex items-center justify-center text-slate-300"><Loader2 className="w-4 h-4 animate-spin" /></div>
            ) : suggestions.length === 0 ? (
              <p className="text-[12px] text-slate-400">{canEdit ? `No open ${out ? 'expenses/bills' : 'invoices'} match this amount.` : 'Sign in to see suggested matches.'}</p>
            ) : (
              <div className="space-y-1.5">
                <p className="text-[12px] text-slate-500">Suggested {out ? 'bills/expenses' : 'invoices'} — matching marks the document paid.</p>
                {suggestions.map((s) => (
                  <div key={`${s.kind}-${s.id}`} className="flex items-center gap-2 rounded-lg ring-1 ring-slate-200/70 p-2.5 hover:ring-slate-300">
                    <div className="min-w-0 flex-1">
                      <div className="text-[13px] font-semibold text-slate-800 truncate">{s.label}</div>
                      <div className="text-[11px] text-slate-400 tabular-nums">${Number(s.amount).toLocaleString()} · {fmtDate(s.date)} · {s.status}</div>
                    </div>
                    {canEdit && (
                      <button onClick={() => doMatch(s.kind, s.id)} disabled={busy === s.id}
                        className="h-7 px-2.5 rounded-md text-[12px] font-bold text-white bg-primary-600 hover:bg-primary-700 inline-flex items-center gap-1.5 disabled:opacity-50">
...
function AddAccount({ privy, onClose, onSaved }: { privy: string | null; onClose: () => void; onSaved: (id?: string) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/40 backdrop-blur-[2px] p-4" onClick={onClose}>
      <div className="w-full max-w-sm bg-white rounded-xl ring-1 ring-slate-200/70 shadow-2xl animate-in fade-in zoom-in-95 duration-150" onClick={(e) => e.stopPropagation()}>
        <div className="h-12 flex items-center justify-between px-4 border-b border-slate-200/70">
          <h2 className="text-sm font-bold text-slate-800">New bank account</h2>
          <button onClick={onClose} className="p-1.5 rounded-md text-slate-400 hover:bg-slate-100"><X className="w-4 h-4" /></button>
        </div>
        <div className="p-4 space-y-3">
          <label className="block"><span className="block text-[12px] font-semibold text-slate-600 mb-1">Name *</span>
            <input autoFocus value={name} onChange={(e) => setName(e.target.value)} placeholder="Business checking" className="w-full h-9 px-2.5 text-[13px] rounded-md bg-white ring-1 ring-slate-200 focus:ring-2 focus:ring-primary-500 outline-none" /></label>
          <div className="grid grid-cols-2 gap-3">
            <label className="block"><span className="block text-[12px] font-semibold text-slate-600 mb-1">Currency</span>
              <input value={currency} onChange={(e) => setCurrency(e.target.value)} className="w-full h-9 px-2.5 text-[13px] rounded-md bg-white ring-1 ring-slate-200 focus:ring-2 focus:ring-primary-500 outline-none uppercase" /></label>
            <label className="block"><span className="block text-[12px] font-semibold text-slate-600 mb-1">Opening balance</span>
              <input type="number" value={opening} onChange={(e) => setOpening(e.target.value)} placeholder="0" className="w-full h-9 px-2.5 text-[13px] rounded-md bg-white ring-1 ring-slate-200 focus:ring-2 focus:ring-primary-500 outline-none" /></label>
          </div>
          <label className="block"><span className="block text-[12px] font-semibold text-slate-600 mb-1">Institution</span>
            <input value={institution} onChange={(e) => setInstitution(e.target.value)} placeholder="Mercury, Revolut…" className="w-full h-9 px-2.5 text-[13px] rounded-md bg-white ring-1 ring-slate-200 focus:ring-2 focus:ring-primary-500 outline-none" /></label>
        ...
        <div className="flex items-center justify-end gap-2 p-3 border-t border-slate-200/70">
          ...
          <button onClick={save} disabled={busy} className="h-8 px-3 rounded-md text-[13px] font-bold text-white bg-primary-600 hover:bg-primary-700 inline-flex items-center gap-1.5 disabled:opacity-50">{busy && <Loader2 className="w-3.5 h-3.5 animate-spin" />} Create</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/40 backdrop-blur-[2px] p-4" onClick={onClose}>
      <div className="w-full max-w-lg max-h-[85vh] flex flex-col bg-white rounded-xl ring-1 ring-slate-200/70 shadow-2xl animate-in fade-in zoom-in-95 duration-150" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {step === 'source' && (
            <div className="space-y-4">
              <label className="block"><span className="block text-[12px] font-semibold text-slate-600 mb-1">Import into account</span>
                <select value={acct} onChange={(e) => setAcct(e.target.value)} className="w-full h-9 px-2.5 text-[13px] rounded-md bg-white ring-1 ring-slate-200 focus:ring-2 focus:ring-primary-500 outline-none">
          ...
          {step === 'map' && (
            <div className="space-y-3">
              <p className="text-[12px] text-slate-500">{rows.length} rows detected. Map your columns:</p>
              {[['txn_date', 'Date'], ['description', 'Description'], ['category', 'Category']].map(([k, label]) => (
                <div key={k} className="flex items-center gap-3"><span className="w-32 shrink-0 text-[13px] font-medium text-slate-700">{label}</span>{sel(k)}</div>
              ))}
              <div className="h-px bg-slate-100 my-1" />
              <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">Amount — one signed column, or separate debit/credit</p>
              <div className="flex items-center gap-3"><span className="w-32 shrink-0 text-[13px] font-medium text-slate-700">Amount (signed)</span>{sel('amount')}</div>
              <div className="flex items-center gap-3"><span className="w-32 shrink-0 text-[13px] font-medium text-slate-700">Money out (debit)</span>{sel('debit')}</div>
              <div className="flex items-center gap-3"><span className="w-32 shrink-0 text-[13px] font-medium text-slate-700">Money in (credit)</span>{sel('credit')}</div>
              {map.debit < 0 && map.credit < 0 && (
                <label className="flex items-center gap-2 text-[12px] text-slate-600 pt-1">
                  <input type="checkbox" checked={flip} onChange={(e) => setFlip(e.target.checked)} className="rounded border-slate-300 accent-primary-600" />
          ...
          {step === 'done' && (
            <div className="py-8 text-center">
              ...
              <p className="text-sm font-bold text-slate-800">Imported {count} transactions</p>
              <p className="text-[12px] text-slate-500 mt-1">{rows.length - count > 0 ? `${rows.length - count} rows were skipped (bad date/amount).` : 'All rows imported.'}</p>
            </div>
          )}
        </div>
        ...
        <div className="shrink-0 flex items-center justify-end gap-2 p-3 border-t border-slate-200/70">
          ...
          {step === 'map' && (
            <button onClick={run} disabled={busy} className="h-8 px-3 rounded-md text-[13px] font-bold text-white bg-primary-600 hover:bg-primary-700 inline-flex items-center gap-1.5 disabled:opacity-50">{busy && <Loader2 className="w-3.5 h-3.5 animate-spin" />} Import {rows.length} rows</button>
          )}
          {step === 'done' && <button onClick={onDone} className="h-8 px-3 rounded-md text-[13px] font-bold text-white bg-primary-600 hover:bg-primary-700">Done</button>}
        </div>
      </div>
    </div>
  );
