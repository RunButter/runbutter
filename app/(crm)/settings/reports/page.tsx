  ...
  return (
    <>
      <header className="h-12 shrink-0 flex items-center gap-3 px-4 border-b border-subtle">
        <h1 className="text-sm font-semibold text-primary flex items-center gap-2"><FileBarChart className="w-4 h-4 text-accent" /> Reports</h1>
        ...
        <div className="max-w-3xl space-y-4">
          ...
          {loading ? (
            ...
            <div className="rounded-xl border border-dashed border-subtle p-12 text-center">
              ...
              <p className="text-[12px] text-tertiary mt-1">Create one to get a PDF in your inbox every week or month.</p>
            </div>
          ) : (
              ...
              {rows.map((s) => (
                <div key={s.id} className="flex items-center gap-3 px-4 py-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="text-[13px] font-semibold text-primary truncate">{s.name}</div>
                  ...
                  <button onClick={() => downloadNow(s)} disabled={downloading} title="Generate and download now"
                    className="h-7 px-2 text-[12px] font-semibold rounded-md ring-1 ring-subtle text-secondary hover:bg-surface-sunken inline-flex items-center gap-1.5 disabled:opacity-40">
                    {downloading ? <Loader2 className="w-3 h-3 animate-spin" /> : <Download className="w-3 h-3" />} PDF
                  </button>
                  {canManage && <>
                    <button onClick={() => setEditing(s)} className="h-7 px-2.5 text-[12px] font-semibold rounded-md ring-1 ring-subtle text-secondary hover:bg-surface-sunken">Edit</button>
          ...
          <div className="flex items-start gap-2 rounded-lg border border-subtle bg-surface-sunken px-3 py-2 text-[12px] text-secondary">
      ...
      <div className="w-full max-w-lg max-h-[88vh] flex flex-col bg-surface rounded-xl ring-1 ring-subtle shadow-popover" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-auto p-4 space-y-4">
          <label className="block">
            <span className="block text-[12px] font-semibold text-secondary mb-1">Report name</span>
            <input value={s.name} onChange={(e) => set({ name: e.target.value })} className={input} />
          </label>

          <div className="grid grid-cols-3 gap-3">
            <label className="block">
              <span className="block text-[12px] font-semibold text-secondary mb-1">Frequency</span>
            ...
            {s.frequency === 'weekly' ? (
              <label className="block">
                <span className="block text-[12px] font-semibold text-secondary mb-1">Day</span>
              ...
              <label className="block">
                <span className="block text-[12px] font-semibold text-secondary mb-1">Day of month</span>
            ...
            <label className="block">
              <span className="block text-[12px] font-semibold text-secondary mb-1">Hour</span>
              <select value={s.hour} onChange={(e) => set({ hour: Number(e.target.value) })} className={input}>
                {Array.from({ length: 24 }, (_, i) => i).map((h) => <option key={h} value={h}>{String(h).padStart(2, '0')}:00</option>)}
              </select>
            </label>
          </div>
          <p className="text-[11px] text-tertiary -mt-2">Times are in {s.timezone} and follow daylight saving.</p>

          <label className="block">
            <span className="block text-[12px] font-semibold text-secondary mb-1">Send to</span>
            ...
            <span className="block mt-1 text-[11px] text-tertiary">Comma-separated.</span>
          </label>

          <div>
            <span className="block text-[12px] font-semibold text-secondary mb-2">What to include</span>
            <div className="space-y-3">
              {GROUPS.map((g) => (
                <div key={g}>
                  <div className="text-[10px] font-semibold uppercase tracking-widest text-tertiary mb-1">{g}</div>
                    ...
                    {SECTION_CATALOG.filter((sec) => sec.group === g).map((sec) => (
                      <label key={sec.id} className="flex items-start gap-2 px-2 py-1.5 rounded-md hover:bg-surface-hover cursor-pointer">
                        ...
                        <span className="min-w-0">
                          <span className="block text-[13px] text-primary">{sec.label}</span>
                          <span className="block text-[11px] text-tertiary leading-snug">{sec.description}</span>
                        </span>
                      </label>
                    ))}
                  </div>
