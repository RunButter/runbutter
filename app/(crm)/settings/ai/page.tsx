  ...
  return (
    <>
      <header className="h-12 shrink-0 flex items-center gap-3 px-4 border-b border-slate-200/70">
        <h1 className="text-sm font-bold text-slate-800 flex items-center gap-2"><Sparkles className="w-4 h-4 text-primary-600" /> AI keys</h1>
      ...
      <div className="flex-1 overflow-auto p-6">
        <div className="max-w-2xl mx-auto space-y-6">
          <div className="flex items-start gap-2 text-[13px] text-slate-500 rounded-xl bg-slate-50 ring-1 ring-slate-200/60 p-3">
          ...
          <div className="rounded-xl bg-white ring-1 ring-slate-200/60 p-4">
            <div className="text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-3">Add a provider key</div>
            <div className="grid sm:grid-cols-2 gap-2.5">
              <label className="block"><span className="block text-[12px] font-semibold text-slate-600 mb-1">Provider</span>
                ...
                {def && <span className="text-[11px] text-slate-400 mt-1 block">{def.help}</span>}
              </label>
              <label className="block"><span className="block text-[12px] font-semibold text-slate-600 mb-1">Model</span>
            ...
            <button onClick={add} disabled={!canEdit || saving} className="mt-3 h-8 px-3 inline-flex items-center gap-1.5 rounded-lg text-[13px] font-bold text-white bg-primary-600 hover:bg-primary-700 shadow-sm disabled:opacity-40">{saving ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Plus className="w-3.5 h-3.5" />} Save key</button>
          </div>
          ...
          <div className="rounded-xl bg-white ring-1 ring-slate-200/60 overflow-hidden">
            {loading ? <div className="h-20 flex items-center justify-center text-slate-300"><Loader2 className="w-5 h-5 animate-spin" /></div>
              : rows.length === 0 ? <div className="px-5 py-8 text-center text-[13px] text-slate-400">No AI keys yet. Add one above to use the Docs assistant.</div>
              : rows.map((r) => (
                <div key={r.id} className={`flex items-center gap-3 px-4 h-14 border-b border-slate-100 last:border-0 ${r.enabled ? '' : 'opacity-50'}`}>
                  ...
                  <button onClick={() => remove(r)} disabled={!canEdit} className="p-1.5 rounded-md text-slate-400 hover:text-rose-600 hover:bg-rose-50 disabled:opacity-40"><Trash2 className="w-4 h-4" /></button>
                </div>
              ))}
          </div>
        </div>
