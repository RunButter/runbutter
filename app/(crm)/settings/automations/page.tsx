    ...
    <>
      <header className="h-12 shrink-0 flex items-center gap-3 px-4 border-b border-subtle">
        ...
        <div className="ml-auto flex items-center gap-1.5">
          ...
          <button onClick={() => setEditing(blank())} disabled={!canEdit} className="h-8 px-3 inline-flex items-center gap-1.5 rounded-lg text-[13px] font-semibold text-inverse-fg bg-inverse hover:bg-inverse/90 shadow-sm disabled:opacity-40" title={!canEdit ? 'Sign in to add' : ''}>
      ...
      <div className="w-full max-w-lg max-h-[90vh] flex flex-col bg-surface rounded-xl ring-1 ring-subtle shadow-popover animate-in fade-in zoom-in-95 duration-150" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="shrink-0 flex items-center justify-between gap-2 p-3 border-t border-subtle">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={save} disabled={saving} className="h-8 px-3 rounded-md text-[13px] font-semibold text-inverse-fg bg-inverse hover:bg-inverse/90 inline-flex items-center gap-1.5 disabled:opacity-50">{saving && <Loader2 className="w-3.5 h-3.5 animate-spin" />} Save</button>
          </div>
        </div>
      </div>
    </div>
...
function Step({ badge, label, tone, children }: { badge: string; label: string; tone: string; children: React.ReactNode }) {
  return (
    <div className="relative pb-4">
      <span className={`absolute -left-[22px] top-0 w-6 h-6 -translate-x-0 rounded-full ${tone} text-accent-fg text-[11px] font-semibold flex items-center justify-center ring-4 ring-canvas`}>{badge}</span>
