    ...
    <>
      ...
      <div className="flex-1 overflow-auto p-6">
        <div className="max-w-4xl mx-auto space-y-6">
          {/* Templates */}
          <div>
            <div className="text-[11px] font-bold uppercase tracking-widest text-slate-400 mb-2">Start from a template</div>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
              {TEMPLATES.map((t) => (
                <button key={t.key} onClick={() => fromTemplate(t.automation)} disabled={!canEdit}
                  className="group text-left rounded-xl bg-white ring-1 ring-slate-200/60 p-3 hover:ring-slate-300 hover:shadow-sm transition-all disabled:opacity-50">
                  <div className={`inline-flex text-[10px] font-bold uppercase tracking-wide px-1.5 py-0.5 rounded-md mb-1.5 ${t.tone}`}>{t.name}</div>
                  <p className="text-[12px] text-slate-500 leading-snug">{t.desc}</p>
                  <span className="mt-1.5 inline-flex items-center gap-0.5 text-[11px] font-semibold text-primary-600 opacity-0 group-hover:opacity-100 transition-opacity">Use template <ArrowRight className="w-3 h-3" /></span>
                </button>
              ))}
            </div>
          </div>
          ...
          {/* Rules */}
          {loading ? (
            <div className="h-24 flex items-center justify-center text-slate-300"><Loader2 className="w-6 h-6 animate-spin" /></div>
          ) : rows.length === 0 ? (
            <div className="rounded-xl ring-1 ring-slate-200/60 bg-white px-6 py-10 text-center">
              <Zap className="w-8 h-8 text-slate-300 mx-auto mb-2" />
              <p className="text-[13px] text-slate-500">No automations yet — pick a template above or build one.</p>
            </div>
          ) : (
                ...
                <div key={a.id} className={`rounded-xl bg-white ring-1 ring-slate-200/60 p-4 ${a.enabled ? '' : 'opacity-60'}`}>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="text-[14px] font-bold text-slate-800 truncate flex items-center gap-1.5"><TI className="w-3.5 h-3.5 text-slate-400" /> {a.name || 'Untitled automation'}</div>
                      <div className="text-[12px] text-slate-500 truncate">
                        {a.trigger_type === 'webhook' ? <>On <b className="text-slate-600">incoming webhook</b></>
                          : a.trigger_type === 'schedule' ? <>Every <b className="text-slate-600">{a.schedule?.every || 'day'}</b></>
          ...
          <div>
            ...
            <div className="rounded-xl bg-white ring-1 ring-slate-200/60 overflow-hidden">
              {runs.length === 0 ? <div className="px-5 py-8 text-center text-[13px] text-slate-400">No runs yet.</div>
                : runs.map((r) => (
...
function WebhookUrl({ token }: { token: string }) {
  ...
  return (
    <div className="mt-2.5 flex items-center gap-2 rounded-lg bg-slate-50 ring-1 ring-slate-200/60 px-2.5 py-1.5">
      <Radio className="w-3.5 h-3.5 text-slate-400 shrink-0" />
      <code className="flex-1 text-[11px] font-mono text-slate-600 truncate">{url}</code>
      <button onClick={() => { navigator.clipboard?.writeText(url); setCopied(true); setTimeout(() => setCopied(false), 1200); }}
        className="h-6 px-2 rounded-md text-[11px] font-semibold text-slate-600 ring-1 ring-slate-200 hover:bg-white inline-flex items-center gap-1">{copied ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />} Copy</button>
    </div>
  );
}
...
function Builder({ automation, privy, connections, onClose, onSaved }: {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/40 backdrop-blur-[2px] p-4" onClick={onClose}>
      <div className="w-full max-w-lg max-h-[90vh] flex flex-col bg-white rounded-xl ring-1 ring-slate-200/70 shadow-2xl animate-in fade-in zoom-in-95 duration-150" onClick={(e) => e.stopPropagation()}>
        <div className="h-12 shrink-0 flex items-center gap-2 px-4 border-b border-slate-200/70">
          <input autoFocus value={a.name} onChange={(e) => set({ name: e.target.value })} placeholder="Automation name" className="flex-1 text-sm font-bold text-slate-800 outline-none placeholder:text-slate-300" />
        ...
        <div className="flex-1 overflow-y-auto p-5">
          <div className="relative pl-7">
            ...
            <Step badge="1" label="Trigger" tone="bg-amber-500">
              <div className="grid grid-cols-3 gap-1.5 mb-3">
                {TRIGGERS.map((t) => (
                  <button key={t.v} onClick={() => set({ trigger_type: t.v })}
                    className={`flex flex-col items-center gap-1 rounded-lg px-1.5 py-2 text-[11px] font-semibold ring-1 transition-colors ${a.trigger_type === t.v ? 'bg-primary-50 ring-primary-300 text-primary-700' : 'ring-slate-200 text-slate-500 hover:bg-slate-50'}`}>
                ...
                <div className="space-y-2">
                  ...
                  {a.conditions.map((c, i) => (
                    <div key={i} className="flex items-center gap-1.5">
                      <span className="text-[11px] text-slate-400 w-8 shrink-0">only if</span>
                      ...
                      <button onClick={() => set({ conditions: a.conditions.filter((_, k) => k !== i) })} className="p-1 rounded text-slate-300 hover:text-rose-600"><X className="w-3.5 h-3.5" /></button>
                    </div>
                  ))}
                  <button onClick={() => set({ conditions: [...a.conditions, { field: '', op: 'eq', value: '' }] })} className="text-[12px] font-semibold text-primary-600 hover:text-primary-700">+ Add filter</button>
                </div>
              )}
              {a.trigger_type === 'webhook' && (
                <div className="space-y-2">
                  <p className="text-[12px] text-slate-500">Any tool can POST JSON to this automation’s URL to trigger it. Reference fields in actions with <code className="bg-slate-100 rounded px-1">{'{{field}}'}</code>.</p>
              ...
              {a.trigger_type === 'schedule' && (
                <div className="flex items-center gap-2">
                  <span className="text-[13px] text-slate-500">Run every</span>
            ...
            {/* Action steps */}
            {a.actions.map((ac, i) => (
              <Step key={i} badge={String(i + 2)} label={`Action ${i + 1}`} tone="bg-primary-600">
                ...
                {ac.type === 'send_email' && (
                  <div className="space-y-2">
                    ...
                    <textarea value={ac.config.body || ''} onChange={(e) => setCfg(i, { body: e.target.value })} rows={3} placeholder="Body — use {{field}}" className="w-full px-2.5 py-2 text-[13px] rounded-md bg-white ring-1 ring-slate-200 focus:ring-2 focus:ring-primary-500 outline-none" />
                  </div>
                )}
                {ac.type === 'create_record' && (
                  <div className="space-y-2">
                    ...
                    <textarea value={ac.config._data || ''} onChange={(e) => setCfg(i, { _data: e.target.value, data: safeJson(e.target.value) })} rows={3} placeholder='{"number":"INV-{{id}}"}' className="w-full px-2.5 py-2 text-[12px] font-mono rounded-md bg-white ring-1 ring-slate-200 focus:ring-2 focus:ring-primary-500 outline-none" />
                  </div>
                )}
            ...
            <div className="relative">
              <span className="absolute -left-[22px] top-1 w-3 h-3 rounded-full bg-white ring-2 ring-slate-300" />
              <button onClick={() => set({ actions: [...a.actions, { type: 'send_webhook', config: {} }] })} className="h-8 px-3 inline-flex items-center gap-1.5 rounded-lg text-[12px] font-bold text-primary-700 ring-1 ring-dashed ring-primary-300 bg-primary-50/50 hover:bg-primary-50"><Plus className="w-3.5 h-3.5" /> Add action</button>
            </div>
          </div>
        ...
        <div className="shrink-0 flex items-center justify-between gap-2 p-3 border-t border-slate-200/70">
          <label className="flex items-center gap-1.5 text-[12px] font-medium text-slate-600"><input type="checkbox" checked={a.enabled} onChange={(e) => set({ enabled: e.target.checked })} className="rounded border-slate-300 accent-primary-600" /> Enabled</label>
          <div className="flex items-center gap-2">
...
function Step({ badge, label, tone, children }: { badge: string; label: string; tone: string; children: React.ReactNode }) {
  return (
    <div className="relative pb-4">
      <span className={`absolute -left-[22px] top-0 w-6 h-6 -translate-x-0 rounded-full ${tone} text-white text-[11px] font-black flex items-center justify-center ring-4 ring-white`}>{badge}</span>
      <div className="rounded-xl ring-1 ring-slate-200/70 bg-white p-3">
