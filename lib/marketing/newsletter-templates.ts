...
export interface NewsletterContent {
  /**
   * The visual builder's document (template `blocks`).
   *
   * Deliberately `any`: the shape belongs to EmailBuilder.js and is validated by
   * their schema at render time. Mirroring it here would be a second source of
   * truth that drifts on their next release, and this file is imported by a
   * route handler that must stay dependency-light. See lib/marketing/email-doc.ts.
   */
  doc?: any;
  /** plain + announcement */
  heading?: string;
...
/**
 * The three FIXED layouts. `blocks` is not here: it renders through
 * `lib/marketing/email-doc.ts` (EmailBuilder.js), which builds its own document
 * rather than filling this file's shell. `renderEmail` in that module is the
 * single entry point callers should use — it dispatches to both.
 */
...
export const TEMPLATE_META: { key: TemplateKey; name: string; description: string }[] = [
  ...
  { key: 'blocks', name: 'Design your own', description: 'A visual builder — blocks, colours, fonts and columns.' },
];
